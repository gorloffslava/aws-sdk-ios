...
IOS_SIMULATOR_ARCHIVE_PATH = f"{PWD}/xcframeworks/output/iOS-Simulator/"
...
def create_mac_catalyst_archive(framework, project_file):
    archive_path = f"{MAC_CATALYST_ARCHIVE_PATH}{framework}"
...
log(f"Creating XCFrameworks in {PWD}")
...
for framework in xcframeworks:
    ...
    create_mac_catalyst_archive(framework=framework, project_file=framework_map[framework])
...
if os.path.exists(MAC_CATALYST_ARCHIVE_PATH):
    shutil.rmtree(MAC_CATALYST_ARCHIVE_PATH)
